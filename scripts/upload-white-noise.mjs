...
const sourceDir = resolve(process.argv[2] ?? 'audio/white-noise');
const bucket = process.env.R2_BUCKET ?? '';
const accountId = process.env.R2_ACCOUNT_ID ?? '';
const accessKeyId = process.env.R2_ACCESS_KEY_ID ?? '';
const secretAccessKey = process.env.R2_SECRET_ACCESS_KEY ?? '';
const keyPrefix = (process.env.WHITE_NOISE_KEY_PREFIX ?? 'white-noise').replace(/^\/|\/$/g, '');
...
if (files.length === 0) {
  throw new Error(`No audio files found in ${sourceDir}`);
}
